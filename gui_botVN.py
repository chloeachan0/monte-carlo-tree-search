...
from mcts_val import ValuedNode
...
class SushiGoGUI(tk.Tk):
    def __init__(self):
        ...
        self.card_buttons = []
        self.stats = {"rounds": [], "final_scores": {}, "winner": None}
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        tk.Label(
            header_frame, text=header, font=("Helvetica", 16, "bold"), bg="#f7f4e9"
        ).pack()

        current_player = (
            "Player 1" if self.state.next_to_move == 1 else "AI Bot (Player 2)"
        )
        tk.Label(
            header_frame,
        ...
        hand = (
            self.state.player1_cards
            if self.state.next_to_move == 1
            else self.state.player2_cards
        )
        ...
        self.selected_label = tk.Label(
            self, text="Selected: None", font=("Helvetica", 12), bg="#f7f4e9"
        )
    ...
    def _make_ai_move(self):
        try:
            root = ValuedNode(state=self.state)
    ...
    def _handle_game_progress(self):
        if self.state.is_game_over():
            ...
            msg = (
                "It's a tie!"
                if res == 0
                else ("Player 1 wins!" if res == 1 else "AI Bot (Player 2) wins!")
            )
    ...
    def _show_played_cards(self):
        ...
        for player, cards in [
            ("Player 1", self.state.player1_cards_chosen),
            ("AI Bot", self.state.player2_cards_chosen),
        ]:
            frame = tk.Frame(chosen_frame, bg="#f7f4e9")
            frame.pack()
            emojis = " ".join(self.state.map_card(c) for c in cards)
            tk.Label(
                frame, text=f"{player}: {emojis}", font=("Helvetica", 12), bg="#f7f4e9"
            ).pack()
